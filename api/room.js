...
function publicRoom(room){return{code:room.code,className:room.className,activeStage:room.activeStage,status:room.status,resultsVisible:room.resultsVisible,version:room.version};}
...
export default async function handler(req,res){res.setHeader('Cache-Control','no-store');try{
 ...
 if(action==='groupSubmit'){
  ...
  const [label,icon]=CONFLICTS[conflictId];await cache().set(key,{groupNum,conflictId,conflictLabel:label,icon,background,result,rule,groupToken,updatedAt:Date.now()},{ttl:TTL});return res.json({ok:true});
 }
 ...
 touch();await saveRoom(room);return res.json({...publicRoom(room),groups:await groups(code),cloud:await cloud(code),teacher:true});
